...
def ResFunctional(
    ...
    # prepare input & output kernel initializers
    activations, def_biasinit, def_kerinit = \
        prepare_default_activations_and_initializers(
    ...
    if kernel_initializer is None:
        input_and_output_kernel_initializer = def_kerinit
    elif isinstance(kernel_initializer, (float, int)):
        input_and_output_kernel_initializer = default_weight_initializer(
            [input_activation] + [output_activation],
            'constant',
            scale=kernel_initializer
        )
    else:
        input_and_output_kernel_initializer = [kernel_initializer for l in [input_activation] + [output_activation]]
    
    # Prepare input & output bias initializers
    if bias_initializer is None:
        input_and_output_bias_initializer = def_biasinit
    elif isinstance(bias_initializer, (float, int)):
        input_and_output_bias_initializer = default_weight_initializer(
            [input_activation] + [output_activation],
            'constant',
            scale=bias_initializer
        )
    else:
        input_and_output_bias_initializer = [bias_initializer for l in [input_activation] + [output_activation]]
        ...
        if isinstance(fld, (str, tuple, list)):
            ...
            output_fields.append(
                Field(
                    name=fld_name,
                    units=fld_units,
                    kernel_initializer=input_and_output_kernel_initializer[-1],
                    bias_initializer=input_and_output_bias_initializer[-1],
                    kernel_regularizer=kernel_regularizer,
                    bias_regularizer=bias_regularizer,
                    trainable=trainable,
                )
    ...
    layer = Dense(
            input_nNeuron,
            kernel_initializer=input_and_output_kernel_initializer[0],
            bias_initializer=input_and_output_kernel_initializer[0],
            kernel_regularizer=kernel_regularizer,
            bias_regularizer=bias_regularizer,
            trainable=trainable,
            name=graph_unique_name("D{:d}b".format(input_nNeuron)),
            **kwargs
        )
    ...
    # Prepare ResNet Blocks
    for i in range(0, res_net_blocks):
        net, layers = ResNetBlock(
                    res_net_hidden_layers = res_net_hidden_layers,
...
def ResNetBlock(
    ...
    **kwargs):
    ...
    # prepare hidden layers.
    if res_net_hidden_layers is None:
        hidden_layers = []
    else:
        hidden_layers = to_list(res_net_hidden_layers)
    if not all([isinstance(n, int) for n in hidden_layers]):
        raise TypeError("Enter a list of integers as the third input assigning layer widths, e.g. [10,10,10]. ")
    ...
    if kernel_initializer is None:
        ...
        kernel_initializer = [kernel_initializer for l in len(hidden_layers) * [activation]]
    ...
    if bias_initializer is None:
        ...
        bias_initializer = [bias_initializer for l in len(hidden_layers) * [activation]]
    # prepare regularizers.
    kernel_regularizer = default_regularizer(kernel_regularizer)
    bias_regularizer = default_regularizer(bias_regularizer)
    
    # # Apply Pre-activation - the same activation with the hidden layers
    input_tensor = net[-1]
    layer = activations[0]
    layers.append(layer)
    activated_input_tensor = layer(input_tensor)

    # Hidden layers of ResNet Block
    for nLay, nNeuron in enumerate(hidden_layers):
        # Add the Layer
        layer = Dense(
            nNeuron,
        ...
        layers.append(layer)
        net[-1] = layer(net[-1])
        if activations[nLay].activation_name != 'linear' and nLay!=len(hidden_layers)-1:
            layer = activations[nLay]
            layers.append(layer)
            net[-1] = layer(net[-1])
    ...
    layer = Add()
    layers.append(layer)
    net[-1] = layer([input_tensor, net[-1]])
